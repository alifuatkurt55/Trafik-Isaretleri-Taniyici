...
# =========================
app = Flask(__name__)
UPLOAD_FOLDER = "backend/dataset/Upload"
MODEL_PATH = "backend/model/traffic_sign_cnn_deep.h5"
IMG_SIZE = (64, 64)
...
def train_model():
    ...
    try:
        history_path = os.path.join("backend", "training_history.json")
...
def cnn_predict(img_or_path, true_label=None):
    ...
    meta_b64 = test_b64
    meta_path = f"backend/dataset/Meta/{prediction}.png"
...
def predict_random_test():
    test_csv = pd.read_csv(os.path.join("backend", "dataset", "Test.csv"))
    row = test_csv.sample(1).iloc[0]
    return jsonify(cnn_predict(os.path.join("backend", "dataset", row["Path"]), int(row["ClassId"])))

@app.route("/predict_batch_test/<int:num_samples>")
def predict_batch_test(num_samples):
    test_csv = pd.read_csv("backend/dataset/Test.csv")
    samples = test_csv.sample(min(num_samples, len(test_csv)))
    results = [cnn_predict(os.path.join("backend", "dataset", row["Path"]), int(row["ClassId"])) for _, row in samples.iterrows()]
...
def confusion_samples(num_samples):
    ...
    for _, row in samples.iterrows():
        res = cnn_predict(os.path.join("backend", "dataset", row["Path"]))
...
def roc_samples(num_samples):
    ...
    try:
        ...
        for _, row in samples.iterrows():
            try:
                img_path = os.path.join("backend", "dataset", row["Path"])
...
def full_test_analysis():
    ...
    try:
        ...
        try:
            save_path = os.path.join("backend", "full_test_results.json")
